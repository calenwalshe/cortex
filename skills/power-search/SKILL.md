...
Unified search skill with cost tracking. Replaces the standalone `/web` and `/ai` skills with a single router backed by the `power-search` Python package. Routes to the best tool based on intent: Tavily for keyword search, Jina for URL reading, Firecrawl for JS scraping, Crawl4AI for site crawling, Perplexity for deep research, Gemini for Google-grounded search, GPT for generation. Tracks cost per query in a local SQLite database.

Package source: https://github.com/calenwalshe/claude-power-search
