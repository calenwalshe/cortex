...
> **STATUS: IMPLEMENTED** — Proposals written by `cortex-research` SKILL Phase 3 (`--phase evals`); plans generated by `scripts/cortex/generate-eval-plan.py` (eval-system-refactor).
...
> **STATUS: IMPLEMENTED** — See `scripts/cortex/format-eval-results.py` (eval-system-refactor). Results are written by `/cortex-eval-run` via `format-eval-results.py`.
...
> **STATUS: ASPIRATIONAL** — Repair recommendation generation and repair contract auto-opening are not yet fully automated. `cortex-review` SKILL scans eval results and emits repair recommendations (implemented in eval-system-refactor); full repair loop automation (auto-open repair contract, auto-re-run evals after repair) is a future slug.
