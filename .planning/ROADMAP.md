...
<details>
<summary>v1.0 Core System (Phases 1-6) -- SHIPPED 2026-03-29</summary>
...
<details>
<summary>v1.1 cortex-idea-stash (Phase 7) -- SHIPPED 2026-04-01</summary>
...
<details>
<summary>v1.2 cortex-discovery-loop (Phases 8-11) -- SHIPPED 2026-04-01</summary>

- [x] **Phase 8: Discovery Loop — Design Docs and Templates** - Write the authoritative design reference and artifact templates for the discovery loop
- [x] **Phase 9: Discovery Loop — Infrastructure Patches** - Patch scaffold, phase guard, and uncertainty register for experiment mode
- [x] **Phase 10: Discovery Loop — /cortex-experiment Skill** - Deliver the 8th Cortex command with open/run/close lifecycle
- [x] **Phase 11: Discovery Loop — Update Existing Skills and Docs** - Update skills and docs to reflect discovery loop and 8-command surface

</details>

<details>
<summary>v1.3 auto-doc-sync (Phase 12) -- SHIPPED 2026-04-02</summary>

- [x] **Phase 12: Auto-Doc-Sync — Pre-Commit LLM Doc Updater** - Build a git pre-commit hook that auto-generates documentation updates from source changes
...
**Goal**: A shared autonomy config exists with three presets (supervised, gates-only, full-auto), a 4-layer resolution chain, mandatory gate enforcement, and backward-compatible defaults -- so all downstream phases have a config substrate to read from
**Depends on**: Phase 12
**Requirements**: AUTON-03, AUTON-04, AUTON-07, AUTON-11
**Contract**: docs/cortex/contracts/adaptive-autonomy/contract-001.md
**Success Criteria** (what must be TRUE):
  1. `templates/cortex/autonomy.json` exists with documented schema: `preset` field, `gates` object with 13 named boolean gates, and inline comments explaining each gate
  2. Config resolution follows 4-layer precedence (invocation flag > project `.cortex/autonomy.json` > global `~/.claude/cortex-autonomy.json` > preset defaults) -- verified by the resolver producing correct merged output for each layer combination
  3. Per-gate overrides in config take precedence over preset defaults (e.g., `preset: "full-auto"` + `gates.contract_approval: true` resolves contract_approval to true)
  4. Mandatory gates (`ux_taste_eval`, `human_action`, `reclarify`) are forced true regardless of preset or per-gate override -- attempting to set them false has no effect
  5. Missing autonomy config defaults to `supervised` preset -- no behavioral change from pre-autonomy system
...
**Contract**: docs/cortex/contracts/adaptive-autonomy/contract-001.md
**Success Criteria** (what must be TRUE):
  1. With `preset: "full-auto"`, the full Cortex pipeline (clarify -> research -> spec -> contract) runs without human stops except at mandatory gates (ux_taste_eval, human_action, reclarify)
  2. With `preset: "supervised"`, all gates fire and the pipeline behaves identically to the pre-autonomy system -- full backward compatibility
...
**Depends on**: Phase 14
**Requirements**: AUTON-05, AUTON-06, AUTON-10
**Contract**: docs/cortex/contracts/adaptive-autonomy/contract-001.md
**Success Criteria** (what must be TRUE):
  1. `/cortex-bridge` reads gsd-handoff.md, spec.md, and contract and generates valid GSD artifacts (PROJECT.md, ROADMAP.md, REQUIREMENTS.md, STATE.md, config.json, CONTEXT.md) in `.planning/`
  2. Bridge-generated ROADMAP.md success criteria match the input contract's done_criteria exactly -- no manual editing needed
...
**Depends on**: Phase 15
**Requirements**: AUTON-08, AUTON-09, AUTON-12
**Contract**: docs/cortex/contracts/adaptive-autonomy/contract-001.md
**Success Criteria** (what must be TRUE):
