...
export async function runLoop(
  ...
  while (true) {
    ...
    if (action.type === 'verify' && telegram) {
    ...
    await telegram?.sendProgress(`Starting: ${action.type}${phaseLabel}`);
    ...
    if (result.stuck) {
      ...
      await telegram?.sendAlert(`Stuck agent detected in session ${result.sessionId}. Halting.`);
      return;
    }
    ...
    await telegram?.sendProgress(`Completed: ${action.type}${phaseLabel} (cost: $${result.costUsd.toFixed(2)})`);
...
export async function main(): Promise<void> {
  ...
  // Initialize optional Telegram bot
  let telegram: TelegramBot | undefined;
  if (config.telegram) {
    telegram = new TelegramBot(config.telegram, logger.telegram);
    telegram.start();
    telegram.startHeartbeat();
    logger.loop.info('Telegram bot started with heartbeat');
  }
  ...
  try {
    await runLoop(config, { telegram, stuckDetector });
  } finally {
    if (telegram) {
      await telegram.stop();
      logger.loop.info('Telegram bot stopped');
    }
  }
